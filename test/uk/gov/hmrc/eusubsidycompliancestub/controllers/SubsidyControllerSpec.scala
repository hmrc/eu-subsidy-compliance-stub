...
import uk.gov.hmrc.eusubsidycompliancestub.models
...
//    }
    ...
    Store.subsidies.put(createUndertakingSubsidies(subsidyUpdate, subsidiesRetrieve.undertakingIdentifier))
      val result: Future[Result] = testResponse[SubsidyRetrieve](
        subsidiesRetrieve,
        "retrieveUndertakingSubsidiesResponse",
        play.api.http.Status.OK
      )
      //val m = Store.subsidies.retrieveSubsidies(subsidiesRetrieve.undertakingIdentifier)
     // val n  = Json.toJson[UndertakingSubsidies](m.get)
      //println("n is ::"+Json.prettyPrint(n))
      val u: JsResult[UndertakingSubsidies] = Json.fromJson[models.UndertakingSubsidies](contentAsJson(result))
      println("u is ::"+u)
